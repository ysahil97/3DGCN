...
for ia in nums:
    ...
    for j in range(int(graph_id),int(graph_id)-5,-1):
        G = nx.read_graphml("../../STWalk/dblp/input_graphs/graph_"+str(j)+".graphml")
...
# print("Done finally")
    ...
    filename = open("result_dblp" + graph_id + ".dat","wb")
    pickle.dump(G_list,filename)
    filename.close()
    filename = open("./labels_3dgcn_dblp" + graph_id + ".dat","wb")
